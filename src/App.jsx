...
import { Home, FileText, Settings, Plus, Users, Package, BarChart3, Wallet, RefreshCw, Receipt, BookOpen, Moon, Sun, Download, X, ShoppingCart, ChevronDown, Building2, Pencil, HelpCircle, Search, Command, Bell, LogOut, LayoutGrid } from 'lucide-react';
import { getAllProfiles, saveProfile, getEnabledModules, getAllBills, getAllProducts, getProfile, getMetaValue, isSupabaseMode } from './store';
...
import MajorGroupsView from './components/MajorGroupsView';
import SubGroupsView from './components/SubGroupsView';
...
import { MASTER_CHILDREN, MASTER_VIEW_IDS, VIEW_MODULE_MAP } from './config/masterMenu';
...
class InvoiceViewErrorBoundary extends React.Component {
  ...
  componentDidCatch(error, info) {
    console.error('New Invoice page error:', error, info);
  }

  render() {
    if (this.state.error) {
      return (
        <div className="glass-panel p-6" style={{ maxWidth: 560, margin: '2rem auto', border: '1px solid #fecaca', background: '#fef2f2' }}>
          <h2 style={{ margin: '0 0 0.5rem', color: '#b91c1c' }}>New Invoice could not load</h2>
          <p style={{ margin: '0 0 1rem', color: '#7f1d1d', fontSize: '0.9rem' }}>
...
function AppContent() {
  const [currentView, setCurrentView] = useState(() => {
    ...
    try {
      ...
      const valid = ['dashboard', 'new', 'clients', 'vendors', 'items', 'brands', 'majorGroups', 'subGroups', 'staff', 'purchaseEntry', 'barcodeLabel', 'inventory', 'expenses', 'purchases', 'recurring', 'receipts', 'reports', 'filing', 'guide', 'settings'];
  ...
  const [masterNavOpen, setMasterNavOpen] = useState(() => {
    try {
      const saved = sessionStorage.getItem('gst_masterNavOpen');
      if (saved != null) return saved === '1';
      const v = sessionStorage.getItem('gst_currentView');
      return MASTER_VIEW_IDS.includes(v);
    } catch { return true; }
  });
  ...
  useEffect(() => {
    ...
    const compute = async () => {
      try {
        ...
        let autoFire = null;
        try {
          const j = isSupabaseMode()
            ? await getMetaValue('lastRecurringAutoFire')
            : (await fetch('/api/meta/lastRecurringAutoFire').then((r) => r.ok ? r.json() : null))?.value;
          if (j && j.date === today && j.count > 0) autoFire = j;
        } catch { /* fine */ }
  ...
  // Cloud mode: load profile whenever auth session is ready / changes (reload-safe)
  useEffect(() => {
    ...
    const loadProfileForSession = async (session) => {
      ...
      try {
        ...
        if (!profileLoaded.current) {
          ...
          if (!p.businessName?.trim() && !localStorage.getItem('freegstbill_onboarded')) {
            setShowWelcome(true);
          }
        }
      } catch (err) {
  ...
  // Local mode: check Express server
  useEffect(() => {
  ...
  // Refresh profile when opening invoice or settings (cloud saves may happen elsewhere)
  useEffect(() => {
    if (!isSupabaseMode() || serverStatus !== 'online') return;
    if (currentView !== 'new' && currentView !== 'settings') return;
  ...
  useEffect(() => {
    if (MASTER_VIEW_IDS.includes(currentView)) setMasterNavOpen(true);
  }, [currentView]);

  useEffect(() => {
    try { sessionStorage.setItem('gst_masterNavOpen', masterNavOpen ? '1' : '0'); } catch { /* ignore */ }
  }, [masterNavOpen]);

  // Pull the user's module preferences once per render.
  ...
  const masterChildrenVisible = MASTER_CHILDREN.filter(c => showIfModule(c.module));
  ...
  const navItems = [
    ...
    { id: 'inventory', icon: Package, label: 'Inventory', module: 'inventory' },
  ...
  const paletteActions = useMemo(() => {
    ...
    masterChildrenVisible.forEach(child => {
      acts.push({ label: `Master › ${child.label}`, hint: '', run: () => setCurrentView(child.id) });
    });
    ...
    <div className="app-layout">
      <div className="sidebar">
        ...
        <nav className="sidebar-nav">
          ...
          {masterChildrenVisible.length > 0 && (
            <div className="nav-group">
              <button
                type="button"
                className={`nav-group-toggle ${masterNavOpen ? 'open' : ''} ${MASTER_VIEW_IDS.includes(currentView) ? 'active' : ''}`}
                onClick={() => setMasterNavOpen(o => !o)}
              >
                <LayoutGrid size={18} /> MASTER
                <ChevronDown size={16} className="nav-group-chevron" />
              </button>
              {masterNavOpen && (
                <div className="nav-group-children">
                  {masterChildrenVisible.map(child => (
                    <button
                      key={child.id}
                      type="button"
                      className={`nav-btn nav-group-child ${currentView === child.id ? 'nav-btn-active' : ''}`}
      ...
      <div className="main-content">
        ...
        {currentView === 'new' && (
          <InvoiceViewErrorBoundary onBack={() => { setEditingBill(null); setCurrentView('dashboard'); }}>
            <InvoiceGenerator
        ...
        {currentView === 'purchaseEntry' && (
          <PurchaseEntryView />
        )}
        {currentView === 'barcodeLabel' && (
          <BarcodeLabelView />
        )}
