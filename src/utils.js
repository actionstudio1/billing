...
export const getLineGrossAmount = (item = {}) => {
  const qty = Math.max(0, Number(item.quantity) || 0);
  const rate = Math.max(0, Number(item.rate) || 0);
  return qty * rate;
};

/** Discount % when set; otherwise derived from legacy flat ₹ discount on old bills. */
export const getLineDiscountPercent = (item = {}) => {
  ...
  const gross = getLineGrossAmount(item);
  const flat = Math.max(0, Number(item.discount) || 0);
  if (gross > 0 && flat > 0) return Math.round((flat / gross) * 10000) / 100;
  return 0;
};

/** Rupee discount from line % (or legacy flat amount). */
export const getLineDiscountAmount = (item = {}) => {
  const gross = getLineGrossAmount(item);
  if (item.discountPercent != null && item.discountPercent !== '') {
    const pct = getLineDiscountPercent(item);
    return Math.min(gross, Math.round(gross * pct / 100 * 100) / 100);
  }
  return Math.min(gross, Math.max(0, Number(item.discount) || 0));
};

/** Post-discount taxable value (before GST is added on top). */
export const getLineTaxableAmount = (item = {}, taxInclusive = false) => {
  const net = Math.max(0, getLineGrossAmount(item) - getLineDiscountAmount(item));
  const taxRate = Math.max(0, Number(item.taxPercent) || 0);
...
export const calculateLineItemTax = (item = {}, taxInclusive = false) => {
  const amount = getLineGrossAmount(item);
...
export const generateEWayBillJSON = (profile, client, details, items, totals, invoiceType) => {
  ...
  const itemList = items.map((item, idx) => {
    const taxable = getLineGrossAmount(item) - getLineDiscountAmount(item);
...
/** Retail item names — uppercase like legacy billing software (e.g. BOYS BLAZER). */
export const normalizeItemName = (name) => String(name || '').trim().toUpperCase();
...
export const FEATURE_GROUPS = [
  ...
  },
  {
    id: 'directory',
    label: 'Directory',
    description: 'Clients, vendors, items and inventory',
    modules: [
      { id: 'clients',   label: 'Clients',   nav: 'clients', core: true },
      { id: 'vendors',   label: 'Vendors / Suppliers', nav: 'vendors', defaultOn: true },
      { id: 'items',     label: 'Items Master', nav: 'items', defaultOn: true },
      { id: 'inventory', label: 'Inventory entries & stock', nav: 'inventory', defaultOn: true },
    ],
  },
  {
    id: 'purchases',
