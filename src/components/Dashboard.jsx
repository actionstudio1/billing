...
import { getAllBills, deleteBill, saveBill, getAllProducts, saveProduct, getProfile, getAllClients, trashPdf, getAllInventoryEntries, getAllItems } from '../store';
import { formatCurrency, INVOICE_TYPES, getCountryConfig } from '../utils';
import { getRetailDashboardSummary } from '../lib/stockReports';
...
export default function Dashboard({ onNew, onEdit, onDuplicate, onConvert }) {
  ...
  const [retailSummary, setRetailSummary] = useState(null);

  const fyOptions = getFYOptions();
  const profileCurrency = getCountryConfig(profile?.country).currency;
  ...
  useEffect(() => {
    ...
    getAllProducts().then(prods => {
      setLowStockProducts(prods.filter(p => {
        const stock = p.stock ?? 0;
        const reorder = parseFloat(p.reorderLevel);
        const limit = Number.isFinite(reorder) && reorder > 0 ? reorder : 5;
        return stock <= limit;
      }));
    }).catch(() => {});
    Promise.all([getAllInventoryEntries(), getAllProducts(), getAllItems(), getAllBills()])
      .then(([entries, prods, items, billData]) => {
        setRetailSummary(getRetailDashboardSummary({
          entries, products: prods, items, bills: billData,
        }));
      }).catch(() => {});
  }, []);
  ...
  const handleDelete = async (bill) => {
    if (confirm('Delete this invoice? This cannot be undone.')) {
      try {
        ...
        trashPdf({ fileName: pdfName, clientName }).catch((err) => console.warn('Could not trash PDF:', err));
    ...
    <div className="dashboard-container">
      <div className="page-header">
        <div>
          ...
          <p className="page-subtitle">Invoices, stock &amp; retail overview</p>
        </div>
          ...
          <div className="stat-card">
            ...
            <div>
              ...
              <h2 className="stat-value" style={{ fontSize: '1.1rem' }}>
                {formatCurrency(retailSummary.totalPurchase, profileCurrency)}
              </h2>
            </div>
          </div>
          <div className="stat-card">
            ...
            <div>
              <p className="stat-label">Total Stock Qty</p>
              <h2 className="stat-value stat-value-green">{retailSummary.totalStockQty}</h2>
            </div>
          </div>
          <div className="stat-card">
            ...
            <div>
              ...
              <h2 className="stat-value stat-value-purple" style={{ fontSize: '1.1rem' }}>
                {formatCurrency(retailSummary.stockValue, profileCurrency)}
              </h2>
            </div>
          </div>
          <div className="stat-card">
            ...
            <div>
              ...
              <h2 className="stat-value stat-value-amber" style={{ fontSize: '1rem' }}>
                {retailSummary.lowStockCount} / {retailSummary.deadStockCount}
              </h2>
            </div>
          </div>
        </div>
      ...
      {lowStockProducts.length > 0 && (
        <div className="overdue-banner" style={{
          ...
          <div style={{ flex: 1 }}>
            <span style={{ fontWeight: 700, color: '#d97706' }}>Reorder Required</span>
            <span style={{ color: '#92400e', marginLeft: 8, fontSize: '0.85rem' }}>
              — {lowStockProducts.length} item{lowStockProducts.length > 1 ? 's' : ''} at or below low stock level
            </span>
          </div>
        </div>
      )}
        ...
        <div className="glass-panel" style={{ marginBottom: '1.25rem', padding: '1rem 1.25rem' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginBottom: '0.75rem' }}>
            ...
            <h3 style={{ margin: 0, fontSize: '0.95rem', fontWeight: 700, color: '#d97706' }}>
              Low Stock Alert — Reorder Required ({lowStockProducts.length} item{lowStockProducts.length > 1 ? 's' : ''})
            </h3>
          </div>
