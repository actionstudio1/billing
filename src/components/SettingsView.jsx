...
export default function SettingsView({ onSaved }) {
  ...
  const [profileLoading, setProfileLoading] = useState(isSupabaseMode());
  ...
  const [barcodePrinter, setBarcodePrinter] = useState(() => getBarcodePrinterConfig());

  const saveBarcodePrinterSettings = () => {
    saveBarcodePrinterConfig(barcodePrinter);
    toast('Barcode printer settings saved', 'success');
  };
  ...
  const loadProfileFromCloud = async () => {
    ...
    try {
      ...
      toast(err?.message || 'Could not load company details', 'error');
    } finally {
  ...
  useEffect(() => {
    const init = async () => {
      ...
      getInvoiceNumberSettings()
        .then(setInvNumSettings)
        .catch((err) => {
          console.error('Invoice number settings load failed:', err);
          toast(err?.message || 'Could not load invoice number settings', 'error');
        });
    };
    init();
  }, []);
  ...
  const saveAccountForm = async () => {
    ...
    try {
      ...
      const saved = result.profile || await getProfile();
      setProfile(saved);
      if (onSaved) onSaved(saved);
      toast(idx >= 0 ? 'Account updated & saved' : 'Account added & saved', 'success');
    } catch (err) {
  ...
  const handleImageUpload = (field, e) => {
    ...
    reader.onload = async (ev) => {
      const updated = { ...profile, [field]: ev.target.result };
      setProfile(updated);
      if (field !== 'logo' && field !== 'signature') return;
      if (!updated.businessName?.trim()) {
        toast('Signature uploaded — enter business name and click Save Profile', 'info');
        return;
      }
      try {
        ...
        setProfile(saved);
        if (onSaved) onSaved(saved);
        const existing = businessProfiles.find(bp =>
          bp.businessName?.trim().toLowerCase() === saved.businessName.trim().toLowerCase()
        );
        if (existing) {
          await saveBusinessProfile({ ...saved, id: existing.id });
          loadBusinessProfiles();
        }
        toast(field === 'signature' ? 'Signature saved — will show on invoices' : 'Logo saved', 'success');
      } catch (err) {
        console.error('Branding auto-save failed:', err);
        toast('Uploaded — click Save Profile to keep it', 'warning');
      }
    };
  ...
  const handleSave = async (e) => {
    ...
    try {
      ...
      toast(
        isSupabaseMode()
          ? 'Company details saved permanently to cloud'
          : 'Profile saved!',
        'success',
      );
    } catch (err) {
  ...
  const handleSaveInvNumSettings = async () => {
    ...
    try {
      ...
      const saved = await getInvoiceNumberSettings();
      setInvNumSettings(saved);
      toast(
        isSupabaseMode()
          ? 'Invoice number format saved to cloud'
          : 'Invoice number settings saved!',
        'success',
      );
    } catch (err) {
      console.error('Invoice number settings save failed:', err);
  ...
  const ALL_BACKUP_PARTS = [
    ...
    { id: 'vendors',        label: 'Vendors',                  hint: 'Supplier / vendor directory' },
  ...
  return (
    <div className="settings-container">
      ...
      {/* ---- Barcode Printer (future-ready) ---- */}
      <div className="glass-panel p-6 mb-6">
        <h3 className="section-title" style={{ marginTop: 0 }}>Barcode Label Printer</h3>
        <p style={{ fontSize: '0.85rem', color: '#64748b', marginBottom: '0.85rem' }}>
          Labels print via browser today. Configure a thermal printer here for future direct printing (ZPL/EPL).
        </p>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '0.75rem' }}>
          <div className="form-group" style={{ margin: 0 }}>
            <label className="form-label">Printer type</label>
            <select className="form-input" value={barcodePrinter.type}
              onChange={e => setBarcodePrinter(p => ({ ...p, type: e.target.value }))}>
              {BARCODE_PRINTER_TYPES.map(t => (
                <option key={t} value={t}>{t === 'browser' ? 'Browser Print (default)' : t.toUpperCase()}</option>
              ))}
            </select>
          </div>
          <div className="form-group" style={{ margin: 0 }}>
            <label className="form-label">Device name</label>
            <input type="text" className="form-input" placeholder="e.g. Zebra ZD220"
              value={barcodePrinter.deviceName || ''}
              onChange={e => setBarcodePrinter(p => ({ ...p, deviceName: e.target.value }))} />
          </div>
          <div className="form-group" style={{ margin: 0 }}>
            <label className="form-label">Label width (mm)</label>
            <input type="number" className="form-input" min="20" max="100"
              value={barcodePrinter.labelWidthMm || 50}
              onChange={e => setBarcodePrinter(p => ({ ...p, labelWidthMm: parseInt(e.target.value, 10) || 50 }))} />
          </div>
          <div className="form-group" style={{ margin: 0 }}>
            <label className="form-label">Label height (mm)</label>
            <input type="number" className="form-input" min="15" max="80"
              value={barcodePrinter.labelHeightMm || 30}
              onChange={e => setBarcodePrinter(p => ({ ...p, labelHeightMm: parseInt(e.target.value, 10) || 30 }))} />
          </div>
        </div>
        <button type="button" className="btn btn-secondary" style={{ marginTop: '0.75rem' }} onClick={saveBarcodePrinterSettings}>
      ...
      <form onSubmit={handleSave} className="glass-panel p-6 mb-6" ref={companyFormRef}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '0.75rem', marginBottom: '1rem' }}>
          <h3 className="section-title" style={{ margin: 0 }}>Company Details</h3>
          <button type="submit" className="btn btn-primary" disabled={saving}>
            <Save size={18} /> {saving ? 'Saving...' : 'Save Profile'}
          </button>
        </div>
        <p style={{ fontSize: '0.8rem', color: 'var(--text-muted)', margin: '0 0 1rem' }}>
          Fill your details, then click <strong>Save Profile</strong>. Data is stored in your Supabase account and survives refresh / logout.
        </p>
        {profileLoading && (
          <p style={{ fontSize: '0.85rem', color: 'var(--text-muted)', marginBottom: '1rem' }}>Loading saved company details…</p>
        )}
        {(() => {
      ...
      <div className="glass-panel p-6 mb-6">
        {/* Invoice Number Format — separate from company form */}
