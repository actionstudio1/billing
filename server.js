...
const DIRS = ['bills', 'clients', 'vendors', 'templates', 'products', 'items', 'brands', 'major_groups', 'sub_groups', 'staff', 'inventory_entries', 'expenses', 'recurring', 'receipts', 'profiles', 'purchases'];
...
app.get('/api/vendors', (req, res) => {
  const vendors = readAllFromDir('vendors');
...
app.post('/api/vendors', (req, res) => {
  ...
  const filePath = path.join(DATA_DIR, 'vendors', safeFileName(vendor.id) + '.json');
  writeJSON(filePath, vendor);
  res.json({ success: true, id: vendor.id });
});

app.delete('/api/vendors/:id', (req, res) => {
  const filePath = path.join(DATA_DIR, 'vendors', safeFileName(req.params.id) + '.json');
  deleteFile(filePath);
  res.json({ success: true });
});
...
app.get('/api/items', (req, res) => {
  const items = readAllFromDir('items');
...
app.post('/api/items', (req, res) => {
  ...
  const filePath = path.join(DATA_DIR, 'items', safeFileName(item.id) + '.json');
  writeJSON(filePath, item);
  res.json({ success: true, id: item.id });
});

app.delete('/api/items/:id', (req, res) => {
  const filePath = path.join(DATA_DIR, 'items', safeFileName(req.params.id) + '.json');
  deleteFile(filePath);
  res.json({ success: true });
});
...
app.get('/api/brands', (req, res) => {
  const brands = readAllFromDir('brands');
...
app.post('/api/brands', (req, res) => {
  ...
  const filePath = path.join(DATA_DIR, 'brands', safeFileName(brand.id) + '.json');
  writeJSON(filePath, brand);
  res.json({ success: true, id: brand.id });
});

app.delete('/api/brands/:id', (req, res) => {
  const filePath = path.join(DATA_DIR, 'brands', safeFileName(req.params.id) + '.json');
  deleteFile(filePath);
  res.json({ success: true });
});
...
// MAJOR GROUP MASTER
// ========================
app.get('/api/major-groups', (req, res) => {
  const rows = readAllFromDir('major_groups');
  rows.sort((a, b) => (a.name || '').localeCompare(b.name || ''));
  res.json(rows);
});

app.post('/api/major-groups', (req, res) => {
  const record = req.body;
  if (!record.id) record.id = 'majgrp_' + Date.now();
  const filePath = path.join(DATA_DIR, 'major_groups', safeFileName(record.id) + '.json');
  writeJSON(filePath, record);
  res.json({ success: true, id: record.id });
});

app.delete('/api/major-groups/:id', (req, res) => {
  const filePath = path.join(DATA_DIR, 'major_groups', safeFileName(req.params.id) + '.json');
  deleteFile(filePath);
  res.json({ success: true });
});
...
// SUB GROUP MASTER
// ========================
app.get('/api/sub-groups', (req, res) => {
  const rows = readAllFromDir('sub_groups');
  rows.sort((a, b) => (a.name || '').localeCompare(b.name || ''));
  res.json(rows);
});

app.post('/api/sub-groups', (req, res) => {
  const record = req.body;
  if (!record.id) record.id = 'subgrp_' + Date.now();
  const filePath = path.join(DATA_DIR, 'sub_groups', safeFileName(record.id) + '.json');
  writeJSON(filePath, record);
  res.json({ success: true, id: record.id });
});

app.delete('/api/sub-groups/:id', (req, res) => {
  const filePath = path.join(DATA_DIR, 'sub_groups', safeFileName(req.params.id) + '.json');
  deleteFile(filePath);
  res.json({ success: true });
});
...
app.get('/api/staff', (req, res) => {
  const staff = readAllFromDir('staff');
...
app.post('/api/staff', (req, res) => {
  ...
  const filePath = path.join(DATA_DIR, 'staff', safeFileName(record.id) + '.json');
  writeJSON(filePath, record);
  res.json({ success: true, id: record.id });
});

app.delete('/api/staff/:id', (req, res) => {
  const filePath = path.join(DATA_DIR, 'staff', safeFileName(req.params.id) + '.json');
  deleteFile(filePath);
  res.json({ success: true });
});
...
// INVENTORY ENTRIES
// ========================
app.get('/api/inventory-entries', (req, res) => {
  const entries = readAllFromDir('inventory_entries');
  entries.sort((a, b) => new Date(b.entryDate || b.createdAt || 0) - new Date(a.entryDate || a.createdAt || 0));
  res.json(entries);
});
...
app.post('/api/inventory-entries', (req, res) => {
  ...
  const filePath = path.join(DATA_DIR, 'inventory_entries', safeFileName(entry.id) + '.json');
  writeJSON(filePath, entry);
  res.json({ success: true, id: entry.id });
});

app.delete('/api/inventory-entries/:id', (req, res) => {
  const filePath = path.join(DATA_DIR, 'inventory_entries', safeFileName(req.params.id) + '.json');
  deleteFile(filePath);
  res.json({ success: true });
});
...
app.get('/api/export', (req, res) => {
  const data = {
    ...
    vendors: readAllFromDir('vendors'),
    termsTemplates: readAllFromDir('templates'),
    products: readAllFromDir('products'),
    items: readAllFromDir('items'),
    brands: readAllFromDir('brands'),
    major_groups: readAllFromDir('major_groups'),
    sub_groups: readAllFromDir('sub_groups'),
    staff: readAllFromDir('staff'),
    inventory_entries: readAllFromDir('inventory_entries'),
...
app.post('/api/import', (req, res) => {
  ...
  if (data.vendors && Array.isArray(data.vendors)) {
    for (const ven of data.vendors) {
      if (ven.id) {
        writeJSON(path.join(DATA_DIR, 'vendors', safeFileName(ven.id) + '.json'), ven);
      }
    }
  }
  ...
  if (data.items && Array.isArray(data.items)) {
    for (const item of data.items) {
      if (item.id) {
        writeJSON(path.join(DATA_DIR, 'items', safeFileName(item.id) + '.json'), item);
      }
    }
  }
  if (data.brands && Array.isArray(data.brands)) {
    for (const brand of data.brands) {
      if (brand.id) {
        writeJSON(path.join(DATA_DIR, 'brands', safeFileName(brand.id) + '.json'), brand);
      }
    }
  }
  if (data.major_groups && Array.isArray(data.major_groups)) {
    for (const rec of data.major_groups) {
      if (rec.id) {
        writeJSON(path.join(DATA_DIR, 'major_groups', safeFileName(rec.id) + '.json'), rec);
      }
    }
  }
  if (data.sub_groups && Array.isArray(data.sub_groups)) {
    for (const rec of data.sub_groups) {
      if (rec.id) {
        writeJSON(path.join(DATA_DIR, 'sub_groups', safeFileName(rec.id) + '.json'), rec);
      }
    }
  }
  if (data.staff && Array.isArray(data.staff)) {
    for (const rec of data.staff) {
      if (rec.id) {
        writeJSON(path.join(DATA_DIR, 'staff', safeFileName(rec.id) + '.json'), rec);
      }
    }
  }
  if (data.inventory_entries && Array.isArray(data.inventory_entries)) {
    for (const entry of data.inventory_entries) {
      if (entry.id) {
        writeJSON(path.join(DATA_DIR, 'inventory_entries', safeFileName(entry.id) + '.json'), entry);
      }
    }
  }
